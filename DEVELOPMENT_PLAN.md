...
## Additional Deliverables (outside the numbered phase plan)

| Deliverable | Scope | Status |
|---|---|---|
| Web workshop tool | A second, separate access mode alongside the MCP server: a server-rendered FastAPI + Jinja2 app (`src/bcm_planner/web/`) for live client workshops, with per-organization access-code tenant isolation (no login system), PDF-only export (WeasyPrint) for the BIA report/BCP/CMT plan, and a Docker + Render deployment (`Dockerfile`, `render.yaml`). Reuses `bia_engine.py`/`bcp_generator.py`/`crisis_management.py` unchanged — no business logic duplicated. One new migration (`schema/004_web_workshop_sessions.sql`). 18 new tests (11 tenant-isolation unit tests + 7 HTTP-level tests including an explicit cross-tenant end-to-end proof and PDF-export validity checks), 156/156 passing total. | **Complete** (2026-09-23). See `CHANGELOG.md` (2026-09-23 entry) and `docs/web_workshop_tool.md` for full detail. |
